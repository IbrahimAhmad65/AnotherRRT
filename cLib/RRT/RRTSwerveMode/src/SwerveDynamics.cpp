...
namespace swerve
{
                               ...
                               const swerve::SwerveConfig &swerveConfig)
  {
    ...
    swerve::Force forceNeeded = {changeInVel.vx * swerveConfig.mass / dt,
                                 changeInVel.vy * swerveConfig.mass / dt};
    return forceNeeded;
  }
  ...
  swerve::Velocity updateModule(swerve::Velocity moduleState, swerve::Force force,
                                ...
                                const swerve::SwerveConfig &swerveConfig)
  {
    swerve::Velocity acceleration = {force.fx / swerveConfig.mass,
                                     force.fy / swerveConfig.mass};
      ...
      swerve::ModulePosition m2, swerve::ModulePosition m3)
  {
    ...
    // find how much torque is applied, where each force is being applied at each
    // module position
                           ...
                           int numRandomForces)
  {
    ...
    for (int i = 0; i < numRandomForces; ++i)
    {
      ...
      double x = r * cos(rho);
      double y = r * sin(rho);
      randomForces.forces.push_back(
          std::make_shared<swerve::Force>(swerve::Force{x * maxR, y * maxR}));
    }
    return randomForces;
  }
                        ...
                        const swerve::SwerveConfig &swerveConfig, double dt)
  {
    ...
    std::cout << swerveState.position.toString() << std::endl;
    ForceNTorque forceNTorque = getTotalForceFromWheelForces(
        forces[0], forces[1], forces[2], forces[3], swerveConfig.m0,
        swerveConfig.m1, swerveConfig.m2, swerveConfig.m3);
    swerve::Velocity module0Vel =
        updateModule(swerveState.m0, forceNTorque.f, dt, swerveConfig);
    swerve::Velocity module1Vel =
        updateModule(swerveState.m1, forceNTorque.f, dt, swerveConfig);
    swerve::Velocity module2Vel =
        updateModule(swerveState.m2, forceNTorque.f, dt, swerveConfig);
    swerve::Velocity module3Vel =
        updateModule(swerveState.m3, forceNTorque.f, dt, swerveConfig);
    newSwerveState.m0 = module0Vel;
    newSwerveState.m1 = module1Vel;
    newSwerveState.m2 = module2Vel;
    newSwerveState.m3 = module3Vel;
    Velocity avg =
        getAverageVelocity(module0Vel, module1Vel, module2Vel, module3Vel);
    ...
    std::cout << newSwerveState.position.toString() << std::endl;
