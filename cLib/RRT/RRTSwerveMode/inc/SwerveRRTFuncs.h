...
//        }
        forces.push_back(getBestForce(start.state.m0,velocities[0],rrtConfig.dt, config));
        forces.push_back(getBestForce(start.state.m1,velocities[1],rrtConfig.dt, config));
        forces.push_back(getBestForce(start.state.m2,velocities[2],rrtConfig.dt, config));
        forces.push_back(getBestForce(start.state.m3,velocities[3],rrtConfig.dt, config));
        SwerveState newState = getUpdatedSwerveState(start.state, forces, config, rrtConfig.dt);
        return newState;
