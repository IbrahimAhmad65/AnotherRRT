...
void rrtPointMassWrapper(const Node &start, const Node &end, RRTPointMassConfig config, std::vector<Node> &arr) {
...
std::vector<Node> rrtPointMassFull(Node &start, const Node &end, RRTPointMassConfig config) {
    std::vector<Node> workerTree;
    std::vector<Node> mainTree;
    std::thread worker1(rrtPointMassWrapper, std::ref(start), std::ref(end), config, std::ref(workerTree));
