...
double getSecondsToTraverse(Cubic xCubic, Cubic yCubic, double maxAccel) {
  ...
  double a1 = yCubic.a;
  double b1 = yCubic.b;
  // Not actually the right number, but has the same x value for all values of
  // a,b a1,b1
  double derivativeOfAccelZero = (-a * b - a1 * b1) / (3 * (a * a + a1 * a1));
  double testMaybe =
      accelMagnitudeFunction(derivativeOfAccelZero, xCubic, yCubic);
  double maxAccelOfOrignialPath =
      std::fmax(testBound1, std::fmax(testBound2, testMaybe));
