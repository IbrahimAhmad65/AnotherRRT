...
std::vector<Node> rrtPointMass(const Node &start, const Node &end,
...
inline void printPath(std::vector<Node> path) {
  if (path.size() > 0) {
    for (int i = 0; i < path.size() - 1; ++i) {
      Node n1 = path[i];
      Node n2 = path[i + 1];
      std::string string = R"(\operatorname{polygon}\left(\left()" +
                           std::to_string(n1.x) + "," + std::to_string(n1.y) +
                           "\\right),\\left(" + std::to_string(n2.x) + "," +
                           std::to_string(n2.y) + "\\right)\\right)";
...
findPathFromTree(const Node &goalNode, const Node &startNode, double maxTime) {
...
inline Node createRandomNode(double maxX, double maxY, double maxVel,
                             int seed) {
...
inline void findNearestNodeOnRange(const Tree &tree, const Node &node,
...
inline Node findOptimalParent(const Tree &tree, const Node &node,
                              double maxVel) {
  double minTime = 1e307;
  Node n;
  for (int i = 0; i < tree.nodes.size(); i++) {
    double timeOfParent = tree.nodes[i].time;
    Cubic xCubic = createCubicFromKnots({tree.nodes[i].x, tree.nodes[i].vx},
                                        {node.x, node.vx});
    Cubic yCubic = createCubicFromKnots({tree.nodes[i].y, tree.nodes[i].vy},
                                        {node.y, node.vy});
